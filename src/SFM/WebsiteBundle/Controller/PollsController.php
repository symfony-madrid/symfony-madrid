...
use SFM\WebsiteBundle\Form\PollType;
...
class PollsController extends Controller {
    ...
    public function indexAction(Request $request) {
        ...
        if (!$poll = $em->getRepository('SFMWebsiteBundle:Poll')->findOneById($request->get('id'))) {
        ...
        if ($request->getMethod() == 'POST') {
            ...
            if ($form->isValid()) {
                ...
                if (!$poll_option = $em->getRepository('SFMWebsiteBundle:PollOption')->findOneById($form_request['options'])) {
                    throw new \Symfony\Component\HttpKernel\Exception\NotFoundHttpException();
                }
                $em->getRepository('SFMWebsiteBundle:Poll')->setHit($poll, $poll_option);
    ...
    public function reportAction(Request $request) {
        ...
        return array(
            'poll' => $poll,
            'poll_options' => $em->getRepository('SFMWebsiteBundle:Poll')->getResult($poll),
        );
    }
}